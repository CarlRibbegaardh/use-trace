...
**Important**: All troublehooting must be done test first. You must create a failing test that reproduces the issue before attempting to fix it.
**Important**: Do small tight focused changes. Verify each change with tests, and explain the steps done, before moving on.
**Important**: Bigger sweeping changes will be rejected and reverted, and we will switch to another copilot model to take over.
**Important**: Build the whole project always. Do not build individual packages or apps in isolation.
