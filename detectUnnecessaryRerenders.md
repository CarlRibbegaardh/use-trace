...
// [TodoItem] Rendering ⚡
// [button] Rendering ⚡
//   ⚠️ Prop change onClick (identical value): (fn:123) → (fn:124)
```
...
// [Form] Rendering ⚡
// [Input] Rendering ⚡
//   ⚠️ Prop change validation (identical value): {required: true, validator: (fn:45)} → {required: true, validator: (fn:46)}
```
...
export function stringify(value: unknown): string {
  try {
    const result = safeStringify(value, (key, val) => {
      // Replace function with unique identity marker
...
interface AutoTracerOptions {
  ...
  enableAutoTracerInternalsLogging?: boolean;
  maxFiberDepth?: number;
  showFunctionContentOnChange?: boolean;
  skipNonTrackedBranches?: boolean;
  skippedObjectProps?: SkippedObjectProp[];
...
describe("walkFiberForUpdates - Identical Value Change Detection", () => {
  ...
  it("should detect identical inline function (different instances, same code)", () => {
    ...
    // Should log: ⚠️ Prop change onClick (identical value): (fn:1) → (fn:2)
  });
  ...
  it("should detect functions in object props", () => {
    const handler1 = () => save();
    ...
    expect(stringify(config1)).not.toBe(stringify(config2));

    // Should log: ⚠️ Prop change config (identical value): {onSave: (fn:1)} → {onSave: (fn:2)}
  });
});
```
