...
This app demonstrates the auto-injection of `useAutoTrace` into React components.
...
**Mode: `opt-out`** - All components get `useAutoTrace()` injected automatically, except:
...
3. `useAutoTrace()` calls are automatically injected as the first statement
...
4. In Sources tab, you can see the transformed code with injected `useAutoTrace()` calls
...
autoTrace.vite({
  ...
  importSource: "use-trace", // Import useAutoTrace from this package
