...
test.describe('Pattern labelHooksPattern Configuration Tests', () => {
  test('should label ALL hooks matching the pattern ^use[A-Z].*', async ({ page, pageLogs }) => {
    await page.goto('/');
    ...
    const counterLogs = pageLogs.filter((log: string) => log.includes('State change counter:'));
    ...
    const allStateLogs = pageLogs.filter((log: string) => log.includes('State change'));
