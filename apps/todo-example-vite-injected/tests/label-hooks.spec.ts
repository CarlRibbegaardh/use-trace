...
test.describe('Hook Labeling E2E Tests', () => {
  test('should label useState, useReducer, and custom hooks based on configuration', async ({ page, pageLogs }) => {
  ...
  await page.click('text=Update Nested Inner Hook');
    ...
    const unlabeledStateLogs = pageLogs.filter((log: string) => log.includes('State change: 2 → 4') || log.includes('State change: 2 → 3'));
    expect(unlabeledStateLogs.length).toBeGreaterThan(0);

    // Pattern test component hooks
    const descriptionLogs = pageLogs.filter((log: string) => log.includes('State change description:'));
    expect(descriptionLogs.length).toBeGreaterThan(0);

    const customHookResultLogs = pageLogs.filter((log: string) => log.includes('State change customHookResult:'));
    expect(customHookResultLogs.length).toBeGreaterThan(0);

    const nestedHookResultLogs = pageLogs.filter((log: string) => log.includes('State change nestedHookResult:'));
