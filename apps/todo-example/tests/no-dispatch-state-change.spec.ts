...
test.describe("State change logging", () => {
  test("No dispatch state-change logs on initial render", async ({
    page,
    pageLogs,
  }) => {
    await page.goto("/");
    ...
    const dispatchLogs = pageLogs.filter((l) =>
      /State change\s+dispatch:/.test(l)
    );

    expect(
      dispatchLogs.length,
      `Expected no "State change dispatch" logs on initial render, got ${
        dispatchLogs.length
      }\n\nDispatch logs:\n${dispatchLogs.join(
        "\n"
      )}\n\nAll logs:\n${pageLogs.join("\n")}`
